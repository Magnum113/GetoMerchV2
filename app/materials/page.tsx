    ...
    <div className="space-y-6 p-4 md:p-6 lg:p-8 min-h-screen bg-muted/30">
      <div className="flex items-center justify-between">
        <div className="space-y-1">
          <h1 className="text-3xl font-bold tracking-tight text-foreground">Управление материалами</h1>
          <p className="text-muted-foreground">Отслеживание сырья и производственных материалов</p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card className="border shadow-sm hover:shadow-md transition-all overflow-hidden group">
          <CardHeader className="flex flex-row items-center justify-between pb-3 pt-3 bg-muted/40">
            <CardTitle className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Всего материалов</CardTitle>
            <div className="h-10 w-10 rounded-lg bg-blue-500/10 flex items-center justify-center group-hover:bg-blue-500/20 transition-colors">
              <Boxes className="h-5 w-5 text-blue-600" />
            </div>
          </CardHeader>
          <CardContent className="pt-6">
            <div className="text-3xl font-bold text-foreground mb-1">{totalMaterials}</div>
            <p className="text-xs text-muted-foreground font-medium">Разных типов</p>
          </CardContent>
        </Card>

        <Card className="border shadow-sm hover:shadow-md transition-all overflow-hidden group">
          <CardHeader className="flex flex-row items-center justify-between pb-3 pt-3 bg-muted/40">
            <CardTitle className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Стоимость запасов</CardTitle>
          ...
          <CardContent className="pt-6">
            <div className="text-2xl font-bold text-foreground mb-1">{Math.round(totalValue).toLocaleString('ru-RU')} ₽</div>
            <p className="text-xs text-muted-foreground font-medium">Общая стоимость</p>
          </CardContent>
        </Card>

        <Card className="border shadow-sm hover:shadow-md transition-all overflow-hidden group">
          <CardHeader className="flex flex-row items-center justify-between pb-3 pt-3 bg-muted/40">
            <CardTitle className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Низкие запасы</CardTitle>
          ...
          <CardContent className="pt-6">
            ...
            <p className="text-xs text-muted-foreground font-medium">Требуют пополнения</p>
          </CardContent>
        </Card>
      </div>
      ...
      {/* Low Stock Alert */}
      {lowStockMaterials > 0 && (
        <Card className="border shadow-sm border-l-4 border-l-amber-500 bg-muted/40">
      ...
      <TableHeader>
        <TableRow className="bg-muted/40 hover:bg-muted/40">
          <TableHead className="font-semibold text-foreground/90">Материал</TableHead>
          <TableHead className="font-semibold text-foreground/90">Домашний склад</TableHead>
          <TableHead className="font-semibold text-foreground/90">Производственный склад</TableHead>
          <TableHead className="font-semibold text-foreground/90">Всего</TableHead>
          <TableHead className="font-semibold text-foreground/90">Цена/Ед.</TableHead>
          <TableHead className="font-semibold text-foreground/90">Общая стоимость</TableHead>
          <TableHead className="font-semibold text-foreground/90">Поставщик</TableHead>
                  ...
                  return (
                    ...
                    >
                      <TableCell className="py-4">
                        <div className="font-semibold text-foreground">{material.material_name}</div>
                      ...
                      <TableCell className="py-4">
                        <span className="font-semibold text-foreground">{Math.round(material.avg_cost_per_unit || 0)} ₽</span>
                      </TableCell>
                      <TableCell className="py-4">
                        <span className="font-bold text-foreground">{Math.round(totalValue).toLocaleString('ru-RU')} ₽</span>
                      </TableCell>
                      <TableCell className="py-4">
                        <span className="text-sm text-muted-foreground">Несколько поставщиков</span>
                      </TableCell>
                      <TableCell className="py-4">
                        <div className="space-y-2">
                          ...
                          <Progress
                            ...
                            className={`h-2 bg-muted/60 w-20 ${isCritical ? "[&>div]:bg-red-500" : isLowStock ? "[&>div]:bg-amber-500" : "[&>div]:bg-green-500"}`}
                          />
                        </div>
                      </TableCell>
                ...
                <TableRow>
                  <TableCell colSpan={13} className="text-center py-12">
                    <div className="flex flex-col items-center gap-3">
                      <Boxes className="h-12 w-12 text-muted-foreground/60" />
                      <p className="text-sm text-muted-foreground font-medium">Материалов не найдено</p>
                    </div>
                  </TableCell>
                </TableRow>
              )}
